...
/// #
/// # if !db_url.starts_with("postgres") { return Ok(()) }
...
/// See the documentation for Postgres for details: [crate::postgres::types]
///
...
///
/// * All methods accept one of `&mut PgConnection`, `&mut Transaction` or `&Pool`.  
/// † Only callable if the query returns no columns; otherwise it's assumed the query *may* return at least one row.
/// ## Requirements
/// * The `DATABASE_URL` environment variable must be set at build-time to point to a Postgres
///   server with the schema that the query string will be checked against.
///   All variants of `query!()` use [dotenvy]<sup>1</sup> so this can be in a `.env` file instead.
///
...
///
/// * The `QueryAs` instance will be bound to Postgres as `query!()` was compiled against it.
///
///     * The schema of the database URL (e.g. `postgres://`) will be used to determine the database type.
///
...
///
/// * change the name of the `DATABASE_URL` variable for using multiple Postgres databases in the same workspace
...
/// [reference `sqlx.toml`]: crate::_config::_reference禁止
...
/// ```
///
/// Bind parameters in the SQL string for Postgres use `$N` where `N` is the 1-based positional argument index.
///
...
///
/// In Postgres you may also use `IS [NOT] DISTINCT FROM` to compare with a possibly `NULL` value.
///
/// ## Nullability: Output Columns
/// In most cases, the Postgres engine can tell us whether or not a column may be `NULL`, and
...
/// Type overrides are available for output columns, utilizing Postgres's support for arbitrary text in column names:
///
/// ##### Force Not-Null
/// Selecting a column `foo as "foo!"` overrides inferred nullability and forces the column to be treated as `NOT NULL`; this is useful e.g. for
...
///
/// ##### Force Nullable
/// Selecting a column `foo as "foo?"` overrides inferred nullability and forces the column to be treated as nullable; this is provided mainly
/// for symmetry with `!`.
///
/// ```rust,ignore
/// # async fn main() {
...
/// ```
///
/// Inference in Postgres is more fragile as it depends primarily on observing
...
/// Selecting a column `foo as "foo: T"` overrides the inferred type which is useful when selecting user-defined [custom types][crate::Type#compile-time-verification]
...
///
/// // Postgres
/// sqlx::query!(r#"select 1 as "id!: MyInt4""#)
///     .fetch_one(&mut conn)
///     .await?;
///
/// // For Postgres this would have been inferred to be `Option<i32>`
...
///
/// ## Offline Mode
/// The macros can be configured to not require a live Postgres connection for compilation,
/// but it requires a couple extra steps:
///
/// * Run `cargo install sqlx-cli`.
/// * In your project with `DATABASE_URL` set (or in a `.env` file) and the Postgres server running,
///   run `cargo sqlx prepare`.
/// * Check the generated `.sqlx` directory into version control.
/// * Don't have `DATABASE_URL` set during compilation.
///
/// Your project can now be built without a Postgres connection (you must omit `DATABASE_URL` or
...
/// project and your Postgres schema itself, run
...
/// the query to ensure it's syntactically and semantically valid for Postgres.
#[macro_export]
#[cfg_attr(docsrs, doc(cfg(feature = "macros")))]
...
/// * The field types must be the Rust equivalent of their Postgres SQL counterparts; see the Postgres
///   module for mappings: [crate::postgres::types]
...
///
/// If you select a column `foo as "foo: _"` it causes that column to be inferred based on the type of the corresponding field in the given
...
///
/// // Postgres
/// sqlx::query_as!(Record, r#"select 1 as "id: _""#)
///     .fetch_one(&mut conn)
///     .await?;
///
/// assert_eq!(record.id, MyInt4(1));
...
/// the Rust type mapping for its corresponding SQL column (see the `types` module of Postgres,
...
/// still does parse the query to ensure it's syntactically and semantically valid for Postgres.
#[macro_export]
#[cfg_attr(docsrs, doc(cfg(feature = "macros")))]
...
/// valid for Postgres, that it only produces one column and that the number of bind parameters
/// is correct.
///
...
/// semantically valid for Postgres, that it only produces one column and that the number of
...
/// See the [configuration guide] and [reference `sqlx.toml`] for details.
///
/// `sqlx-cli` can also read these options and use them when setting up or migrating Postgres databases.
///
